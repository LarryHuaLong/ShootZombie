	...
	if (m_dwFlags == WAVEFILE_READ)
	{
		...
		if (NULL == m_hmmio)
		{
			...
			if (NULL == (hResInfo = FindResource(NULL, strFileName, L"WAVE")))
			{
				...
				{
					MessageBox(NULL, L"FindResource", L"Error", MB_OK);
					return NULL;
				}
			}
			if (NULL == (hResData = LoadResource(GetModuleHandle(NULL), hResInfo)))
			{
				MessageBox(NULL, L"LoadResource", L"Error", MB_OK);
				return NULL;
			}
			if (0 == (dwSize = SizeofResource(GetModuleHandle(NULL), hResInfo)))
			{
				MessageBox(NULL,L"SizeofResource", L"Error", MB_OK);
				return NULL;
			}
			if (NULL == (pvRes = LockResource(hResData)))
			{
				MessageBox(NULL,L"LockResource", L"Error", MB_OK);
				return NULL;
			}
			m_pResourceBuffer = new CHAR[dwSize];
			if (m_pResourceBuffer == NULL)
			{
				MessageBox(NULL, L"new", L"Error", MB_OK);
				return NULL;
			}
